# log-subagent-stop.ps1 (S17 D16-bis fix A102 + A106 v2 ISO8601)
# Hook SubagentStop : capture nom agent depuis payload JSON stdin.
# Contrat empirique identique a log-subagent-start.ps1.
...
try {
    ...
    if ($agentName -like 'unknown:*') {
        try {
            ...
            $diagLines = @()
            $diagLines += '========================================='
            $diagLines += ('[A106-DIAG-STOP] ' + $ts + ' fallback=' + $agentName)
            $diagLines += ('  PID: ' + $PID)
    ...
    $ts = Get-Date -Format 'yyyy-MM-ddTHH:mm:ss'
    Add-Content -Path $logPath -Value ('[AGENT STOP] ' + $ts + ' - Agent: ' + $agentName + ' termine')
} catch { }
exit 0
