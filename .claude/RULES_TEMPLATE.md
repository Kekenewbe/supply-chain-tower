...
template_source: {SOURCE_PROJECT}/.claude/RULES_TEMPLATE.md
generated_at: {GENERATION_DATE}
---

# Regles non-negociables — {PROJECT_NAME}

> Snapshot operationnel des regles imposees a tous les agents du projet {PROJECT_NAME}.
> Source : {SOURCE_PROJECT}/.claude/RULES_TEMPLATE.md (v1.0)
...
- **Index Pinecone** : `{PROJECT_NAME}-memory` (lowercase kebab, isolation memoire stricte)
...
- INTERDIT : modifier directement {SOURCE_PROJECT} pour le projet {PROJECT_NAME} derive (utiliser `new-project.ps1`).
- OBLIGATOIRE : fingerprint cles API au demarrage (banner check), jamais la cle complete.
- OBLIGATOIRE : anti-leak — verifier `{SOURCE_PROJECT}` absent du repo {PROJECT_NAME} (critere portabilite).
